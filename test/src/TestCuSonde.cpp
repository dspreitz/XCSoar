...
/** what CuSonde.cpp uses, in K per metre */
static constexpr double DALR = 0.00974;
/** the threshold CuSonde.cpp applies to the thermal index */
static constexpr double TI_THRESHOLD = -1.6;
 ...
 */
static void
FlyTheSounding(CuSonde &cu_sonde, bool terrain_valid) noexcept
{
  ...
  calculated.terrain_valid = terrain_valid;
  calculated.terrain_altitude = terrain_valid ? AIRFIELD_MSL : 0;
...
main()
{
  ...
  const double expected_thermal_height_agl =
    (TI_THRESHOLD - (SURFACE_TEMPERATURE_C - FORECAST_MAX_C))
    / (DALR - MIXED_LAPSE);

  /* T_dry(h) = T_d(h): T_max - DALR h = T_ds - DEWPOINT_LAPSE h */
  const double expected_cloud_base_agl =
    (FORECAST_MAX_C - SURFACE_DEWPOINT_C) / (DALR - DEWPOINT_LAPSE);
  ...
  for (unsigned level : {4u, 14u, 24u}) {
    ...
    ok1(fabs(cu_sonde.cslevels[level].dry_temperature.ToCelsius()
             - (FORECAST_MAX_C - DALR * h_agl)) < 0.01);
  }
  ...
  /* without a terrain file the elevation is unknown: the reset value
     is kept, and the caller's altitude_agl is not mistaken for it */
  CuSonde no_terrain;
  no_terrain.Reset();
