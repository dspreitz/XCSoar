...
internal one of your Android or iOS device if it has one.
...
\item[Aircraft acceleration] (load factor)  Used for netto variometer
  ...
  On Android, XCSoar uses the phone's accelerometer when no external
...
find the ceiling.  The dry adiabat starts at the terrain elevation
below the glider, so a terrain file is needed for the forecast to be
right away from sea level.  If the atmosphere is stable, no ceiling is
found and the analysis page shows ``---'' in its place.
...
rate.  If no clouds are forecast, no cloud base is found and the
analysis page shows ``---'' in its place.
...
the pilot is invited to consult \url{http://www.drjack.info} on how RASP 
...
You can find a list of available RASP forecasts at \url{http://www.drjack.info/RASP}.
...
You can either install RASP data files yourself, or use XCSoar's built-in automatic
download for many regions worldwide via the 'Site Files / Site Files'
configuration page, \ref{sec:site-files}, or the \emph{Update} button in
Info $\rightarrow$ Weather $\rightarrow$ RASP.  In that RASP panel,
\emph{Auto update} (on by default) also refreshes an out-of-date or missing
file when you open a RASP map page or when auto time has no data; turn it
off for offline use. 

The files have \verb|.dat| extension and the information is shown 
as colour-coded terrain overlays on the map.  \\ \\
Make sure that terrain display is set to ``on''. \tip{}  \\ \\

The main configuration dialog for RASP is in the weather setup via menu
\menulabelr{\bmenug{Info}\blink\bmenug{Weather Setup}}
\bmenuw{RASP}.
You select the RASP data file to display, and the data field to be shown.
After choosing the the configuration, use \bmenuw{Assign to page} to 
assign that configuration to the current map page.

Alternatively, you can control which pages show weather data and how weather display
is controlled via the page configuration (Section~\ref{conf:screenpages}):
Set \emph{Map overlay} to \emph{RASP},
choose the \emph{RASP layer} for that page, and optionally set the
\emph{Bottom area} to \emph{Weather controls} for in-flight forecast time
control. This allows configuration of several pages quickly.

\begin{center}
\includegraphics[angle=0,width=0.8\linewidth,keepaspectratio='true']{figures/dialog-weather.png}
\end{center}

When the bottom \emph{Weather controls} panel is visible, \emph{Auto advance}
...
RASP display configuration parameters are in the \bmenuw{RASP} tab in
\menulabelr{\bmenug{Info}\blink\bmenug{Weather Setup}}. Global options are:
\begin{description}
\item[File] Shows the currently selected RASP data file and allows selection 
and download of other RASP data files. 
\item[Modified] Shows the last modification date of the currently selected RASP data file.
\item[Update] Attempts to download the latest version of the currently selected RASP data file.
This is available for RASP data files that were downloaded from the XCSoar repository.
THe download is in the background, the data will automatically update once it is completed.
\item[Layer] Selects the RASP data field to display on the currently active map page, 
indicated above the layer field.
\item[Colormap] Below the layer selector, a colormap preview is shown for the currently
selected RASP data field, with the scale given in the currently selected units.
\item[Contours] Allows activation/deactivation of contour lines in the weather data,
as well as the density of the contour lines.
\item[Opacity] Allows adjustment of the opacity of the weather overlay. Some weather layers
are global and do not have a transparency channel, such as the surface temperature. This
setting allows to make them partially transparent to enable the underlying terrain to
shine through.\\
For RASP layers with their own transparency souch as wave lift or cloud cover, this
setting only reduces opacity, it never overrides the transparency of that layer.
\item[Assign to page] Assigns the currently selected RASP configuration to the currently
active map page.
\item[Add page] Creates a new map page with the currently selected RASP configuration after
...
\begin{center}
\includegraphics[angle=0,width=0.8\linewidth,keepaspectratio='true']{figures/rasp-wave.png}
\end{center}
...
\begin{center}
\includegraphics[angle=0,width=0.8\linewidth,keepaspectratio='true']{figures/rasp-rain.png}
\end{center}
...
\item[BL wind spd] 
The speed and direction of the vector-averaged wind in the boundary layer.\@  This
prediction can be unreliable if there is large wind shear through the BL.\@
...
\section{Weather forecast, EDL Soaring}\label{sec:weather-edl}

XCSoar can display wave-oriented weather overlays provided by the
public EDL Soaring MBTiles service at
\url{https://www.edl-soaring.com/mbtiles/}.  XCSoar downloads the
tiles on demand from EDL Soaring's public extraction service and stores
them locally as \verb|.mbtiles| files in the weather cache.

Before using EDL, enable \emph{EDL Weather Page} in the system
configuration under \emph{System setup / Weather} (requires an OpenGL build
with HTTP support).

Tiles are cached locally.  Open the \bmenuw{EDL} tab in
\menulabelr{\bmenug{Info}\blink\bmenug{Weather}} or the EDL section in
\emph{System setup / Weather} to manage the cache:
\begin{description}
\item[Cached day] Show which UTC days are already stored in the local
  cache.  A day is marked \emph{Complete} when all 24 forecast hours and
  all five EDL levels are available locally, otherwise it is marked
  \emph{Partial}.
\item[Precache day] Download all 24 hours and all five EDL levels for
  ...
  in flight.
\item[Clean other days] Keep only the selected cached UTC day and delete
  the other locally cached EDL days.
\end{description}

Configure a map page in the page configuration dialogue
(Section~\ref{conf:screenpages}): set \emph{Main area} to \emph{Map}
(or \emph{Map (north-up)}), \emph{Map overlay} to \emph{EDL}, and
\emph{Bottom area} to \emph{Weather controls}.  The in-flight bottom
panel provides:
\begin{description}
\item[Auto advance] When enabled, XCSoar follows the next UTC forecast
  ...
  is active.
\item[Forecast] Select the forecast hour to display.  XCSoar shows this
  ...
  timestamps.
\item[Level] Select one of the supported pressure levels.  The EDL
...
\emph{Map overlay} to \emph{EDL} in the page configuration.
